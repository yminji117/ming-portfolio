...
export async function verifyGuestbookPassword(
  ...
  if (error) {
    if (error.message.includes("locked")) {
      return { ok: false, message: "5회 실패로 10분간 잠겼어요. 잠시 후 다시 시도해 주세요." };
    }
  ...
  if (!result?.ok) {
    const { data: statusRows } = await supabase.rpc("guestbook_auth_status", {
      p_id: entryId,
    });
    const failCount = (statusRows as { fail_count: number }[] | null)?.[0]?.fail_count ?? 0;
    return {
      ok: false,
